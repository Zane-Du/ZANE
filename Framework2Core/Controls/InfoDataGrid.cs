    ...
    public partial class InfoDataGrid : UserControl
    {
        ...
        public void LoadConfigs()
        {
          ...
          //  this.LoadObjConfigsFromIni(this.Name);
            ...
            this.LoadObjConfigsFromIni("InfoDataTable0");


            // 如果列字典中没有 [序号]、[时间]，则添加
            列字典.Clear();

            if (!列字典.ContainsKey("速度"))
            {
                列字典.Add("速度", "string");
            }
            ...
            //if (!列字典.ContainsKey("上限"))
            //{
            //    列字典.Add("上限", "string");
            //}
            //if (!列字典.ContainsKey("下限"))
            //{
            //    列字典.Add("下限", "string");
            //}
            //if (!列字典.ContainsKey("重量"))
            //{
            //    列字典.Add("重量", "string");
            //}
            if (!列字典.ContainsKey("温度"))
            {
                列字典.Add("温度", "string");
            }
            if (!列字典.ContainsKey("批次"))
            {
                列字典.Add("批次", "string");
            }
            if (!列字典.ContainsKey("目标值"))
            {
                列字典.Add("目标值", "string");
            }
            if (!列字典.ContainsKey("密度"))
            {
                列字典.Add("密度", "string");
            }
            if (!列字典.ContainsKey("密封圈"))
            {
                列字典.Add("密封圈", "string");
            }
            if (!列字典.ContainsKey("操作员"))
            {
                列字典.Add("操作员", "string");
            }

            if (!列字典.ContainsKey("实际值"))
            {
                列字典.Add("实际值", "string");
            }
        ...
        public void AddOneRow(Dictionary<string, string> productInfo)
        {
            ...
            if (启用数据库)
            {
                ...
                DatabaseManager.EnqueueRecord("InfoDataTable0", ColumnList, valueList);//数据库的表名，列集合，值集合
